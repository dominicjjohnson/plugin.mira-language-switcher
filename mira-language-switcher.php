...
class Mira_Language_Switcher {
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(
            'mira-language-switcher',
            __('WPML Import', 'mira-language-switcher'),
            __('WPML Import', 'mira-language-switcher'),
            'manage_options',
    ...
    public function detect_language() {
        ...
        // Update the cookie so other functions (auto_redirect, menu selection) see
        // the correct language rather than a stale value from a previous page visit.
        if (!headers_sent()) {
            setcookie('mira_language', $default_language, time() + (30 * 24 * 60 * 60), '/');
        }
        $_COOKIE['mira_language'] = $default_language;
    ...
    public function menu_flags_css() {
        ...
        .menu-item-language-switcher .current-lang img {
            opacity: 0.45;
        }
    ...
    private function get_language_url($target_lang) {
        ...
        if (!$default_page_id) {
            // No translation link found — fall back to same slug in target language.
